...
public class ProgressBarCircularIndeterminate extends CustomView {
    ...
    public ProgressBarCircularIndeterminate(Context context, AttributeSet attrs) {
        super(context, attrs);
        setAttributes(attrs);
        backgroundColor = Color.parseColor("#4394ec");
    }

    // Set atributtes of XML to View
    protected void setAttributes(AttributeSet attrs) {

        setMinimumHeight(GlobalUtil.dp2px(32));
        setMinimumWidth(GlobalUtil.dp2px(32));
    ...
    private void drawFirstAnimation(Canvas canvas) {
        if (radius1 < getWidth() / 2) {
            ...
            if (cont >= 50) {
                ...
                radius2 = (radius2 >= getWidth() / 2 - GlobalUtil.dp2px(4)) ? (float) getWidth() / 2 - GlobalUtil.dp2px(4) : radius2 + 1;
            }
            temp.drawCircle(getWidth() / 2, getHeight() / 2, radius2, transparentPaint);
            canvas.drawBitmap(bitmap, 0, 0, new Paint());
            if (radius2 >= getWidth() / 2 - GlobalUtil.dp2px(4))
                cont++;
...
//		temp.drawARGB(0, 0, 0, 255);
        ...
        transparentPaint.setColor(getResources().getColor(android.R.color.transparent));
        transparentPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
        temp.drawCircle(getWidth() / 2, getHeight() / 2, (getWidth() / 2) - GlobalUtil.dp2px(4), transparentPaint);
